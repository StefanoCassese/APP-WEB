export function mainCartControler() {
    ...
    document.addEventListener('click', (event) => {
        ...
        if (!(event.target === $btnCarrito || $btnCarrito.contains(event.target))
        && !(event.target === cartContainer || cartContainer.contains(event.target))
        && !event.target.closest('.cart-item') && !(event.target === $btnCarritoHamb))
        {
