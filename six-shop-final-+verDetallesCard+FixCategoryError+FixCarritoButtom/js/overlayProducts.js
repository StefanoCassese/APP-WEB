...
export function generateProductsOverlay(productId, productsList) {
            ...
            <div class="product-card-container">    
                ...
                <section class="product-card-information" >
                    ...
                    <div class="products-card-btns product__button ">
                        <button class="add-to-cart-overlay btn btn-color-2" data-productId="${product.id}">Agregar al Carrito</button>
                    </div>
                </section>
            </div>         
        </article>
    ...
    const $btnAddToCart = document.querySelector('.add-to-cart-overlay')

    $btnAddToCart.addEventListener( 'click', ()=> {
        const id = $btnAddToCart.getAttribute('data-productId')
        addToCart(id)
        $newProductCardOverlay.style.display = 'none';
    })
